...
import { sendEmail } from "../utils/sendEmail"; // ✅ EmailJS helper
          ...
          <div className="grid md:grid-cols-2 gap-10 items-start">
            ...
            <div>
              ...
              <ul className="mt-4 list-disc ml-5 space-y-1 text-[#F5EADF]/95">
                <li>
                  {fr
                    ? "15 € en journée (jusqu’à 4 personnes)"
                    : "€15 daytime (up to 4 people)"}
                </li>
...
function ArrivalForm({ fr }) {
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.name || !form.phone) {
      setMsg(
        fr ? "Veuillez remplir les champs obligatoires." : "Please fill the required fields."
      );
      return;
    }

    setLoading(true);
    const res = await sendEmail(form, "Services");
    setLoading(false);
  ...
  return (
    ...
    >
      <FormInput label={fr ? "Nom" : "Name"} name="name" value={form.name} onChange={onChange} placeholder={fr ? "Nom" : "Name"} />
      <FormInput label={fr ? "Téléphone" : "Phone"} name="phone" value={form.phone} onChange={onChange} placeholder="+212…" />
      <FormInput label={fr ? "Numéro de vol" : "Flight number"} name="flight" value={form.flight} onChange={onChange} placeholder={fr ? "Ex. AT123" : "e.g. AT123"} />
      <FormInput label={fr ? "Ville de départ" : "Departure city"} name="city" value={form.city} onChange={onChange} placeholder={fr ? "Ex. Madrid" : "e.g. Madrid"} />
      <FormInput label={fr ? "Date d’arrivée" : "Arrival date"} name="arrival_date" type="date" value={form.arrival_date} onChange={onChange} />
      <FormInput label={fr ? "Heure d’arrivée" : "Arrival time"} name="arrival_time" type="time" value={form.arrival_time} onChange={onChange} />
      ...
      <div className="col-span-2">
        ...
        <textarea
          ...
          placeholder={
            fr ? "Observations, demandes spéciales…" : "Notes, special requests…"
          }
        />
      </div>
      ...
      <div className="col-span-2 flex justify-center pt-2">
        <button
          type="submit"
          disabled={loading}
          className="btn-gold mt-3 px-8 py-3 rounded-lg bg-[#E9C46A] text-[#264653] font-semibold shadow transition hover:bg-[#264653] hover:text-[#E9C46A]"
        >
          {loading ? (fr ? "Envoi..." : "Sending...") : fr ? "Envoyer" : "Send"}
        </button>
      </div>

      {msg && (
        <p className="col-span-2 mt-2 text-center text-sm text-brand-teal">
          {msg}
        </p>
      )}
    </form>
